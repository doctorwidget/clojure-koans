...
(meditations
  "To categorize a collection by some function, use group-by." ; group-by builds __maps__ 
  ...
  (= {1 [{:id 1 :name "Bob"} {:id 1 :last-name "Smith"}] ; it uses the fn to find key for each 
      2 [{:id 2 :name "Jennifer"}]}  ; and the value for each key is a vector
